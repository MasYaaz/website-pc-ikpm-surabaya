<script lang="ts">
	...
	import MediaSidebar from '$lib/component/ui/MediaSidebar.svelte'; // Import Sidebar
	import { ArrowLeft, Send, Save, Image as ImageIcon, Loader2 } from 'lucide-svelte';
	import { fade } from 'svelte/transition';

	let title = $state('');
	let content = $state('');
	let excerpt = $state('');
	let featuredImageId = $state<number | null>(null);
	let previewUrl = $state('');

	// Media Library States
	let showMedia = $state(false);
	let mediaTarget = $state<'thumbnail' | 'editor'>('editor');
	let editorInstance = $state<any>(null); // Untuk akses fungsi insertImage di Editor

	// Fungsi buka media untuk Thumbnail
	function openMediaForThumbnail() {
		mediaTarget = 'thumbnail';
		showMedia = true;
	}

	// Fungsi pemicu dari tombol di toolbar Editor (opsional jika ingin integrasi penuh)
	function openMediaForEditor() {
		mediaTarget = 'editor';
		showMedia = true;
	}

	// Handler saat gambar dipilih dari Sidebar
					...
					>
						...
						{:else}
							...
							>
								...
								<p class="mt-2 text-[10px] font-bold uppercase">Pilih dari Media</p>
							</div>
						{/if}
					</button>
					<p class="mt-3 text-[10px] text-stone-400 italic">Rekomendasi: 1200x630px (Max 2MB)</p>
				</div>
